...
def send_email(
    ...
    password: str = "your-app-password"  # SMTP password (should come from config)
        ...
        try:
            ...
            # Add CC recipients if provided(CC is used for carbon copy that means the email is sent to the CC recipients but they are not the main recipients)
            if cc:
                msg['Cc'] = ', '.join(cc)  # Set CC recipients as comma-separated string
            
            # Attach email body as HTML content because it allows for rich formatting 
                ...
                for file_path in attachments:
                    if os.path.exists(file_path):  # Check if attachment file exists
                        with open(file_path, "rb") as attachment:  # Open file in binary mode
                            part = MIMEBase('application', 'octet-stream')  # Create attachment part this means the file is treated as binary data
...
def search_emails(
    ...
    """Search emails by query. Use get_unread_emails for full IMAP search."""
    try:
        return get_unread_emails.invoke({
            "limit": limit,
            "folder": folder,
        })
    except Exception as e:
        logger.warning(f"Email search failed: {str(e)}")
        return []
